...
/*!
 ...
 @brief     Elasticsearchとインターフェースするクラスのファクトリクラス
 @note      本クラスはElasticsearchとインターフェースするクラスのファクトリクラスである。ElasticsearchクラスとでAbstruct
 Factoryパターンを形成している。 また、Elasticsearchクラスは、TcpClientを必要としているためそのインスタンスも生成している。
*/
...
public:
  ...
  @brief  デフォルトコンストラクタ
  */
  ElasticsearchFactory() : analyzer_(nullptr)
  {
  ...
  */
  static const bool isDestroyed() { return ElasticsearchFactory::destroyed_; }
      ...
      if (!analyzer_)
      {
        ...
        if (ip != "0.0.0.0")
        {
          analyzer_ = dynamic_cast<measurementor::IAnalyzer*>(
            new Elasticsearch(std::make_shared<::TcpClient>(ip, port), apiKey, version, std::move(index), ip.get(), port.get()));
        }
        else
        {
          analyzer_ = dynamic_cast<measurementor::IAnalyzer*>(
            new Elasticsearch(std::make_shared<::TcpClient>(host, port), apiKey, version, std::move(index), host.get(), port.get()));
        }
      }
    }
    return analyzer_;
...
private:
  ...
  std::map<std::string, std::string>
    setting_;  //!< confファイルから読み込んだElasticsearchサーバに関する設定（ホスト名、IPアドレス、接続先ポート、API Key）
};
